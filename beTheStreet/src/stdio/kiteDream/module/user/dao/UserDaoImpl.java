...
public class UserDaoImpl implements UserDao {
	...
	public List<User> getUserByParam(String param, String value) {
		try {
			@SuppressWarnings("unchecked")
			List<User> list = getSessionFactory().getCurrentSession().createCriteria(User.class).add(Restrictions.eq("active", true)).add(Restrictions.eq(param.trim(), value.trim())).setResultTransformer(CriteriaSpecification.DISTINCT_ROOT_ENTITY).list();
			if (list == null) {
				return new ArrayList<User>();
			}
			return list;
	...
	public List<User> manageSearch(String keyword) {
		keyword = "%" + keyword + "%";
		@SuppressWarnings("unchecked")
		List<User> list = getSessionFactory().getCurrentSession().createQuery("from User user where user.active=true and user.name like '"+keyword+"' or user.nickname like '"+keyword+"'").list();
		if (list == null) {
			return new ArrayList<User>();
		}
		return list;
