...
public class CreateXL {
	...
	public static File createUserExcel(List<User> users) {
		...
		try {
			...
			//group type 	group org 	group name 	group creater
			...
			for(User user: users){
				...
				cell = row.createCell(1);
				cell.setCellType(HSSFCell.CELL_TYPE_STRING);
				cell.setCellValue(Constant.TIME.format(user.getCreate_time()));
				cell = row.createCell(2);
				cell.setCellType(HSSFCell.CELL_TYPE_STRING);
				cell.setCellValue(user.getCoins().getGreenNum());
				cell = row.createCell(3);
				cell.setCellType(HSSFCell.CELL_TYPE_STRING);
				cell.setCellValue(user.getCoins().getYellowNum());
				cell = row.createCell(4);
				cell.setCellType(HSSFCell.CELL_TYPE_STRING);
				cell.setCellValue(user.getCoins().getRedNum());
				cell = row.createCell(5);
				cell.setCellType(HSSFCell.CELL_TYPE_STRING);
